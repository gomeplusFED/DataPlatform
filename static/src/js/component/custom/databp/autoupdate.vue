		...
		<div class="nform-box">
			<ul class="clearfix">
				<li class="clearfix">
					...
					<label>站点</label>
					<select class="form-control inp inpW1"
					        id="website"
					        data-content="请选择站点"
					        v-model="searchParam.website">
						<option v-for="(i,site) of sites"
						        :value="site.url"
						        :selected="site.url === searchParam.website ? 'selected' : ''">{{site.name}}</option>
					</select>
					<label>版本号</label>
					...
					<button class="btn"
					        type="button"
					        @click="queryClick">查看更新日志</button>
				</li>
				<li>
					<button class="btn btn-searchLi-top btn-primary"
					        type="button"
					        @click="runUpdate">一键更新</button>
				</li>
			</ul>
		</div>
		<div v-show="dataList.length || noData"
		     class="list-cot list-cot-h list-group">
